...
def load_trades_from_notion(token: str, database_id: str, page_size: int = 100) -> pd.DataFrame:
    ...
    out["Session"]     = df.apply(lambda r: _first_nonempty(r, SESSION_FIELDS), axis=1)
    ...
    out["PnL"]         = pd.to_numeric(df.apply(lambda r: _first_nonempty(r, PNL_FIELDS), axis=1), errors="coerce")
